...
if(isset($update))
{
	$select=$db->query("select * from admin where email='".$_SESSION['admin_email']."' ");
	...
	if($res['password']==$op)
	{
		...
		{
		$db->query("update admin SET password='$np' where email='".$_SESSION['admin_email']."'");
...
<form method="post">
<?php if(isset($msg)){?>
<div class="form-group" style="background:#F9F9F9;padding:5px">
...
<div class="form-group">
	<div class="input-group input-group-sm">
	 ...
	 <input type="text" readonly="readonly" class="form-control"  placeholder="<?php echo ucfirst($_SESSION['admin_email']);?>"/>
	</div>  
   </div>
...
<div class="form-group">
  ...
  <button type="submit" name="update" class="btn btn-default">Update Password</button>
</form>
</div>
